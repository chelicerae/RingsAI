...
class Pinterest_Helper(object):
    ...
    def chech_more_ideas(self):

        more_ideas_list = self.browser.find_elements_by_tag_name('h5')

        more_ideas_check = True if len(list(filter(lambda x: 'More ideas' in x.text, more_ideas_list))) > 0 else False

        return more_ideas_check
            ...
            while threshold > 0:
                try:
                    ...
                    if tries > persistence:
                        ...
                        return final_results, final_descriptions, final_info
                    for i in images:
                        src = i.get_attribute("src")
                        desc = i.get_attribute("alt")
                        if src:
                            if src.find("/236x/") != -1:
                    ...
                    final_descriptions = list(set(final_descriptions + descriptions))

                    #When the board finishes
                    more_ideas = self.chech_more_ideas()
                    if more_ideas:
                        threshold = 0
